...
/**
 * A factory providing each concrete role in the ARIA specification. Each role implements methods
 * that a GWT application can use to modify the appropriate DOM attributes for that role.
 *
 ...
 * set the ARIA <i>label</i> property to the help text that the screen reader can use:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Most ARIA properties have a prefix of "aria-" and only affect screen readers, but there is one
 * exception: the "tabindex" property is the same one used for tab order in regular widgets. For
 ...
 * </pre>
 *
 * <p>When the button changes state (it's enabled or disabled), we can change the ARIA enabled state
 * to match, like this:
 *
 * <pre>
 ...
 * same state as someone viewing the page visually.
 *
 ...
 *   <li>{@link CompositeRole}, for widgets that contain other widgets.
 *   <li>{@link StructureRole}, for DOM elements that are part of the page layout.
 *   <li>{@link LandmarkRole}, for DOM elements defining common regions of a page.
 *   <li>{@link WindowRole}, for DOM elements that define windows and dialogs.
 * </ol>
 *
...
public final class Roles {
  ...
  private static final MenuitemcheckboxRole MENUITEMCHECKBOX =
      new MenuitemcheckboxRoleImpl("menuitemcheckbox");
  ...
  public static Role roleOf(Element element) {
    ...
    return Arrays.stream(roleAttributeValue.split("\\s+"))
        .map(ROLES_MAP::get)
